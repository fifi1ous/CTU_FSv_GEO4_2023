...
el=[0.7*RAD,0.7*RAD,5];          %přesnost měření v pořadí s_směrAB[mgon], s_směrAP[mgon], s_délkaAP[mm]
ex=[30,30,30,30];                %přenost podkladu v pořadní sxB[mm],syB[mm],sxB[mm],syB[mm]
%% nastavení
X=2;Y=1;
...
A1=[-((P(Y)-A(Y))/(SAP^2)),(P(X)-A(X))/(SAP^2);(P(X)-A(X))/SAP,(P(Y)-A(Y))/SAP];
A2=[((P(Y)-A(Y))/(SAP^2))-((B(Y)-A(Y))/(SAB^2)),-(P(X)-A(X))/(SAP^2)+(B(X)-A(X))/(SAB^2),((B(Y)-A(Y))/(SAB^2)),-(B(X)-A(X))/(SAB^2)
    -(P(X)-A(X))/SAP,-(P(Y)-A(Y))/SAP,0,0];
