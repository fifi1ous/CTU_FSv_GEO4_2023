...
el=[0.7*RAD,0.7*RAD,5];          %přesnost měření v pořadí s_směrPA[mgon], s_směrPB[mgon], s_délkaPA[mm]
...
A1=[((B(Y)-P(Y))/(SBP^2))-((A(Y)-P(Y))/(SAP^2)),-(B(X)-P(X))/(SBP^2)+(A(X)-P(X))/(SAP^2);-(A(X)-P(X))/SAP,-(A(Y)-P(Y))/SAP];

A2=[((A(Y)-P(Y))/(SAP^2)),-(A(X)-P(X))/(SAP^2),-((B(Y)-P(Y))/(SBP^2)),(B(X)-P(X))/(SBP^2)
    (A(X)-P(X))/SAP,(A(Y)-P(Y))/SAP,0,0];
