...
AORSM=[A1Y,A1X,ones(mer_sm,1),zeros(mer_sm,2*poc_pb)];

if byl_podrobny_bod==1 
A1YPB=1./(((X(1) - PB_body(:,1)).^2./(X(2) - PB_body(:,2)).^2 + 1).*(X(2) - PB_body(:,2)));
A1XPB=-(X(1) - PB_body(:,1))./(((X(1) - PB_body(:,1)).^2./(X(2) - PB_body(:,2)).^2 + 1).*(X(2) - PB_body(:,2)).^2);
AYsmPB=-1./(((X(1) - PB_body(:,1)).^2./(X(2) - PB_body(:,2)).^2 + 1).*(X(2) - PB_body(:,2)));
AXsmPB=(X(1) - PB_body(:,1))./(((X(1) - PB_body(:,1)).^2./(X(2) - PB_body(:,2)).^2 + 1).*(X(2) - PB_body(:,2)).^2);
pom2=[];
for i=1:poc_pb
    pom1=zeros(poc_pb,2);
    pom1(i,1)=AYsmPB(i);
...
A0PBSM=[A1YPB,A1XPB,ones(1*poc_pb,1),pom2];
else
A0PBSM=[];
end
...
AORD=[A2Y,A2X,zeros(mer_d,1),zeros(mer_d,2*poc_pb)];

if byl_podrobny_bod==1 
A2YPB=(2*X(1) - 2* PB_body(:,1))./(2*sqrt((X(2) -  PB_body(:,2)).^2 + (X(1) -  PB_body(:,1)).^2));
A2XPB=(2*X(2) - 2* PB_body(:,2))./(2*sqrt((X(2) -  PB_body(:,2)).^2 + (X(1) -  PB_body(:,1)).^2));
AYdPB=-(2*X(1) - 2*PB_body(:,1))./(2*sqrt((X(2) -  PB_body(:,2)).^2 + (X(1) -  PB_body(:,1)).^2));
AXdPB=-(2*X(2) - 2*PB_body(:,2))./(2*sqrt((X(2) -  PB_body(:,2)).^2 + (X(1) -  PB_body(:,1)).^2));
pom2=[];
for i=1:poc_pb
    pom1=zeros(poc_pb,2);
    pom1(i,1)=AYdPB(i);
...
A0PBD=[A2YPB,A2XPB,zeros(poc_pb,1),pom2];
else
A0PBD=[];
end
...
if byl_podrobny_bod==1
    for i=1:poc_pb
    smlPB(i)=atan2(PB_body(i,1)-X(1),PB_body(i,2)-X(2));
    if smlPB(i)<0
        smlPB(i)=smlPB(i)+2*pi;
    end
    smlPB(i)=smlPB(i)-X(3);
    if smlPB(i)<0
       smlPB(i)=smlPB(i)+2*pi;
...
if byl_podrobny_bod==1
    dlPB=sqrt((PB_body(:,1)-X(1)).^2 + (PB_body(:,2)-X(2)).^2);
else
    dlPB=[];
end
...
if poc_pb>1
pom1=[];
pom2=reshape(X(4:end),[poc_pb,poc_pb]);
for i=1:poc_pb
    pom1=[pom1;pom2(:,i)'];
end
PB_body=pom1;
...
if byl_podrobny_bod==1
for i=1:poc_pb
cPB(i)=sqrt((smXPB(i)-smYPB(i))^2+4*(Ex(3+i,4+i)^2));
aPB(i)=sqrt((smXPB(i)+smYPB(i)+cPB(i))/2)
bPB(i)=sqrt((smXPB(i)+smYPB(i)-cPB(i))/2)
alfaPB(i)=(atan2(2*Ex(3+i,4+i),smXPB(i)-smYPB(i))/2)/RAD;
if alfaPB(i)<0
    alfaPB(i)=alfaPB(i)+400;
end
end
cPB
aPB
